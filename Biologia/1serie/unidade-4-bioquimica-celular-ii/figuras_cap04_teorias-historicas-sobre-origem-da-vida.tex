...
\begin{tikzpicture}[>=Latex,node distance=0.75cm]
  ...
  \node[box,fill=yellow!18,right=0.8cm of past] (bio) {biogênese\\vida vem de vida};
  \draw[very thick,->] (ge) -- (redi);
  \draw[very thick,->] (redi) -- (spal);
  \draw[very thick,->] (spal) -- (past);
  \draw[very thick,->] (past) -- (bio);
  \node[align=center,below=0.65cm of spal] {controle experimental corrige conclusões apressadas};
\end{tikzpicture}
\end{document}
