...
\begin{tikzpicture}[>=Latex,node distance=0.75cm]
  ...
  \node[box,fill=orange!12,right=1.0cm of gam] (pun) {4 x 4\\16 combinações};
  \node[box,fill=yellow!18,right=1.0cm of pun] (prop) {9:3:3:1\\fenótipos};
  \draw[very thick,->] (aabb) -- (gam);
  \draw[very thick,->] (gam) -- (pun);
  \draw[very thick,->] (pun) -- (prop);
  \node[align=center,above=0.35cm of pun] {genes independentes + dominância completa};
\end{tikzpicture}
\end{document}
