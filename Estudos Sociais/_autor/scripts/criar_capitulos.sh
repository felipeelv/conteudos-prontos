...
#
# Disciplina coberta: Estudos Sociais.
...
DISCIPLINAS_VALIDAS=("Estudos Sociais")
...
# --- Coletar capítulo-modelo aprovado da FAIXA correspondente ao ano/série ---
# Usar como referência de tom/ritmo/densidade. NUNCA copiar estrutura literal — vence prompt-autor.
# Mapeamento ANO → modelo. Cada modelo cobre um perfil etário; cruzar faixas contamina densidade
# e tamanho de parágrafo (ex.: usar modelo de EF2 em 4º ano produz texto longo demais).
case "$ANO" in
  4ano|5ano)             FAIXA_MODELO="modelo-ef1-finais.md" ;;
  6ano|7ano|8ano|9ano)   FAIXA_MODELO="modelo-ef2.md" ;;
  1serie|2serie|3serie)  FAIXA_MODELO="modelo-em.md" ;;
  *)                     FAIXA_MODELO="" ;;
esac

declare -a MODELOS=()
if [[ -n "$FAIXA_MODELO" && -f "$PROMPT_DIR_FULL/$FAIXA_MODELO" ]]; then
  MODELOS+=("$PROMPT_DIR_FULL/$FAIXA_MODELO")
fi
...
elif [[ -n "$FAIXA_MODELO" ]]; then
  echo "  Modelos aprovados: nenhum (faixa esperava $FAIXA_MODELO — só prompt-autor calibra)"
fi
