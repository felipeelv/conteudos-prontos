...
\begin{tikzpicture}[>=Latex,node distance=0.7cm]
  ...
  \node[box,fill=yellow!20] (comb) {combustível};
  ...
  \node at (tri.corner 2) [below left] {combustível};
  ...
  \node[align=center,below=0.55cm of tri] {retirar um lado do triângulo apaga o fogo};
\end{tikzpicture}

\begin{tikzpicture}[>=Latex,node distance=0.85cm]
  \tikzstyle{box}=[draw,rounded corners=2pt,very thick,minimum width=2.9cm,minimum height=0.85cm,align=center]
  \node[box,fill=blue!10] (o2ok) {$O_2$ suficiente};
  \node[box,fill=green!12,right=1.0cm of o2ok] (prodok) {$CO_2 + H_2O$\\chama azul};
  \node[box,fill=orange!12,below=0.9cm of o2ok] (o2low) {pouco $O_2$};
  \node[box,fill=red!10,right=1.0cm of o2low] (prodlow) {$CO +$ fuligem\\chama amarela};
  \draw[->,thick] (o2ok) -- (prodok);
  \draw[->,thick] (o2low) -- (prodlow);
  \node[align=center,below=0.55cm of o2low] {a quantidade de oxigênio\\muda os produtos formados};
\end{tikzpicture}
\end{document}
