...
\begin{tikzpicture}[>=Latex,node distance=0.9cm]
  ...
  \node[box,fill=yellow!18] (fonte) {fonte primaria\\recurso natural};
  \node[box,fill=blue!10,right=1.5cm of fonte] (conv) {conversao\\usina ou aparelho};
  ...
  \node[box,fill=green!8,below=1.0cm of fonte] (ren) {renovavel\\repoe em escala humana};
  \node[box,fill=gray!15,below=1.0cm of uso] (naoren) {nao renovavel\\estoque finito};
  \draw[thick,->] (fonte.south) -- (ren.north);
  \draw[thick,->] (fonte.south) .. controls +(0,-1.0) and +(-1.0,0.5) .. (naoren.north west);
  \node[align=center,below=0.3cm of conv] {classificar exige criterio};
\end{tikzpicture}
...
\begin{tikzpicture}[x=0.11cm,y=0.22cm,>=Latex]
  ...
  \foreach \y/\name/\value/\color in {
    18/Hidreletrica/56.8/blue!65,
    12/Eolica/12.2/green!55!black,
  ...
  \node[align=center,above] at (42,23) {Exemplo: matriz eletrica brasileira renovavel em 2023};
  \node[align=center,below] at (42,-4) {dados ajudam, mas nao encerram os trade-offs};
\end{tikzpicture}
