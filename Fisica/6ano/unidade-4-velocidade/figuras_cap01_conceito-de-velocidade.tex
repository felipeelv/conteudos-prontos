...
\begin{tikzpicture}[x=0.055cm,y=1cm,>=Latex]
  ...
  \node[align=center] at (110,-1.05) {Comparar velocidade pede\\distância e tempo juntos.};
\end{tikzpicture}
...
\begin{tikzpicture}[>=Latex,node distance=1.1cm]
  ...
  \node[box,fill=blue!8] (d) {distância\\$d=100\,\mathrm{m}$};
  \node[box,fill=red!8,below=of d] (t) {tempo\\$t=10\,\mathrm{s}$};
  \node[box,fill=green!10,right=2.4cm of $(d)!0.5!(t)$] (v) {velocidade média\\$v=10\,\mathrm{m/s}$};

  \draw[very thick] ($(d.south west)+(0,-0.1)$) -- ($(d.south east)+(0,-0.1)$);
  \node[left=0.35cm of $(d)!0.5!(t)$] {$\div$};
  \draw[very thick,->] ($(d)!0.5!(t)+(1.7,0)$) -- (v.west);

  \node[align=center,below=0.7cm of t] {$v=\frac{d}{t}$\\velocidade = distância dividida pelo tempo};
\end{tikzpicture}
