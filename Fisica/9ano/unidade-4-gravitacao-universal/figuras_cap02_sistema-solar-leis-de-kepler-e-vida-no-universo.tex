...
\begin{tikzpicture}[scale=0.78,>=Latex]
  ...
  \foreach \r/\name/\ang/\color in {1.2/Mercurio/30/gray,1.8/Venus/140/orange,2.5/Terra/230/blue,3.5/Marte/310/red}{
    \draw[thick,\color] (0,0) ellipse ({\r} and {0.62*\r});
    \fill[\color] ({\r*cos(\ang)},{0.62*\r*sin(\ang)}) circle (0.11);
    \node[font=\small] at ({\r*cos(\ang)+0.45},{0.62*\r*sin(\ang)+0.18}) {\name};
  }
  \draw[->,very thick] (0,0) -- (2.5,0) node[midway,above] {$1\ \mathrm{UA}$};
  \node[align=center] at (0,-2.8) {quanto maior a distância média, maior o periodo orbital};
\end{tikzpicture}
...
\begin{tikzpicture}[>=Latex,node distance=1cm]
  ...
  \node[align=center,below=0.45cm of lei] {Kepler relaciona periodo e distância média};
\end{tikzpicture}
