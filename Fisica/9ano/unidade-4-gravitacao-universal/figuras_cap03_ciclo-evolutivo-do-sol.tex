...
\begin{tikzpicture}[>=Latex,node distance=0.7cm]
  ...
  \node[above=0.35cm of seq,align=center] {estabilidade por equilíbrio};
...
\begin{tikzpicture}[>=Latex,node distance=0.75cm]
  ...
  \node[box,fill=green!10,below left=0.9cm and 1cm of estrela] (média) {massa média\\como o Sol};
  \node[box,fill=orange!12,below right=0.9cm and 1cm of estrela] (massiva) {massa muito\\maior};
  \node[box,fill=red!12,below=0.8cm of média] (gv) {gigante vermelha};
  ...
  \draw[very thick,->] (estrela) -- (média);
  \draw[very thick,->] (estrela) -- (massiva);
  \draw[very thick,->] (média) -- (gv);
