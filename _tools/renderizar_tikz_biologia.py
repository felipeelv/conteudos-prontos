#!/usr/bin/env python3
"""Renderiza os TikZ standalone de Biologia usados nos capítulos."""
...
def render(path: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="tikz-biologia-") as tmp:
        tmp_path = Path(tmp)
        subprocess.run(
            [
                "pdflatex",
        ...
        for page in expected_pages(path):
            subprocess.run(
                [
                    "pdftoppm",
                    "-png",
